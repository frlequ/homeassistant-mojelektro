...
from .const import (
    ...
    CONF_METER_ID,
    CONF_DECIMAL,
)  # Assuming you define CONF_DECIMAL in your .const module
...
class MojeElektroFlowHandler(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_user(self, user_input=None):
        ...
        if user_input is not None:
            ...
            decimal = user_input.get(
                CONF_DECIMAL
            )  # This returns None if CONF_DECIMAL is not in user_input
            api = MojElektroApi(
                user_input[CONF_TOKEN], user_input[CONF_METER_ID], decimal, session
            )
