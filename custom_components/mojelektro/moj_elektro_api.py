...
class MojElektroApi:
    ...
    setupTagArray = '[ {"oznaka": "A+_T0", "sensor": "daily_input"}, {"oznaka": "A+_T1", "sensor": "daily_input_peak"}, {"oznaka": "A+_T2", "sensor": "daily_input_offpeak"}, {"oznaka": "A-_T0", "sensor": "daily_output"}, {"oznaka": "A-_T1", "sensor": "daily_output_peak"}, {"oznaka": "A-_T2", "sensor": "daily_ouput_offpeak"}]'
    ...
    def getData(self):
        ...
        if (cache == None ):
            ...
            sensorReturn.update(self.sensors_output(cache.get('15'),json.loads(self.setupTag15Array)))
            sensorReturn.update(self.sensors_output(cache.get('meter'),json.loads(self.setupTagArray)))
            ...
            #Check if 15min and daily matches
            target_reading_type = "32.0.2.4.1.2.12.0.0.0.0.0.0.0.0.3.72.0"
            index = self.find_tag(target_reading_type, cache.get("15"), 4)

            
            sum15MinPart = sum(float(item['value']) for item in cache.get("15")[index]['intervalReadings'][10:96])
            sum15Min = round(sum(float(item['value']) for item in cache.get("15")[index]['intervalReadings'][0:96]), 5)
